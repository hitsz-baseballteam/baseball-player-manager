# Documentation

Start from [../AGENTS.md](../AGENTS.md) for the high-level map.
This directory holds the deeper project records.

## Index

| Path | Purpose |
|---|---|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Observed current-state architecture |
| [PLANS.md](./PLANS.md) | Planning workflow and plan placement |
| [design-docs/](./design-docs/index.md) | ADRs and core beliefs |
| [product-specs/](./product-specs/index.md) | Feature specifications |
| [DESIGN.md](./DESIGN.md) | Visual and interaction guidance |
| [FRONTEND.md](./FRONTEND.md) | Frontend conventions and migration notes |
| [SECURITY.md](./SECURITY.md) | Security constraints |
| [RELIABILITY.md](./RELIABILITY.md) | Reliability expectations |
| [QUALITY_SCORE.md](./QUALITY_SCORE.md) | Quality tracking |
| [exec-plans/](./PLANS.md) | Active/completed plans and tech debt |
| [references/](./references/) | External reference docs for agents (`-llms.txt`) |
| [generated/](./generated/) | Generated artifacts and session history |

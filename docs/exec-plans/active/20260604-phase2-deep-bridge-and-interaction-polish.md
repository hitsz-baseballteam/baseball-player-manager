...
## 当前开放问题
1. **destructive action 边界**
   - 已确认：本 slice 不把删除方案放到首页，也不做对应桥接
   - 重置示例数据仍保持不前置
2. **提醒映射粒度**
   - 首版用规则映射足够；不做复杂语义解析
3. **桥接层抽不抽单独文件**
   - 如果只增加 3~5 个桥接点，可留在 `PlayerManagerClient`
   - 如果增长到动作 + 面板定位 + 高亮规则，建议抽 `legacy-bridge` 文件

## 下一步
本计划确认后，再进入实现：
- 先做 bridge 层与 focus/highlight
- 再补首页动作和跳转入口
- 最后做交互 polish 与测试

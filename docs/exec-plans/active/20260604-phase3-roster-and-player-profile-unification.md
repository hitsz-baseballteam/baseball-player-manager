...
## 验证
- [ ] `/roster` 路由可访问并通过解锁校验
- [ ] `/roster` 的筛选、计数、球员卡浏览正常
- [ ] `/roster` 上可新增球员
- [ ] `/roster` 上可批量编辑与批量删除
- [ ] 单删 / 批量删除后，scenario 分配会同步清理
- [ ] 能从 `/roster` 打开抽屉或进入完整档案页
- [ ] `/players/[playerId]` 保存、冲突刷新、空态保持正常
- [ ] drawer/page 的主要字段分组一致
- [ ] 首页 / 名册 / 球员档案三者导航链路完整
- [ ] `npm test`
- [ ] `npm run lint`
- [ ] `npm run build`
...
## 当前工作假设
1. `/players/[playerId]` 在 Phase 3 就接入统一壳层/导航语言；若实现中发现页面密度或返回链路明显受损，再回退到局部 chrome 方案。
2. 抽屉与 page 继续共用 `PlayerProfileEditor` 主体，只在外层容器与导航控件上分化。

## 当前建议
在已确认“完整名册工作台 + 抽屉优先”前提下，建议继续采用：
- **独立档案页在 Phase 3 就接入统一壳层/导航语言**

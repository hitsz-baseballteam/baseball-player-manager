...
### Slice 2D — 深度桥接与交互细化（待实施）
- 执行计划：`docs/exec-plans/active/20260604-phase2-deep-bridge-and-interaction-polish.md`
- 建立结构化 bridge 层（动作触发 + 面板定位 + 高亮反馈）
- 扩展首页动作：导出、方案辅助动作、提醒/指标/阵容概览的精确跳转
- 补齐桥接与面板高亮测试
...
- [x] `npm run build`
- [ ] 手动检查：桌面端首页首屏、移动端信息折叠、主题切换后的警报 / 指令条 / 指标卡一致性

## 当前仍保留的实现级注意点
1. **直接桥接的优先级排序**：首波优先确保自动排阵 / 新增球员 / 导入数据 / 方案切换这几类高频动作，其中低频动作如果桥接成本异常高，可顺延。
2. **轻量方案切换器的边界**：只负责切换当前方案，不在本切片内扩展为完整方案 CRUD 面板。
3. **最近变更区块的文案纪律**：只能表达“最近更新时间 / 当前状态说明”，不能写成看似真实但仓库里并不存在的活动流。

> 页面设计方向和首波产品取舍已确认，可以进入 2B 实现切片。

...
> 2026-06-18 状态：**TD-10 全部 P0 + P1 项已落地**。P0-1（客户端 snapshot 缓存）、P0-2（Server Component cache + Cache-Control）、P0-3（动态连接池 max）、P1-1（unnest 批量 INSERT）、P1-2（unstable_cache 短窗口缓存）、P1-3（REPEATABLE READ 隔离级）均已实现。`writeNormalizedWorkspace` 中所有 9 个子表写入已改用 `insertRowsWithUnnest` → `prepareUnnestArgs` 路径，不再是 for-loop 逐行 INSERT。下文保留了历史分析和实施记录，但凡与当前代码不一致时，以仓库代码为准。此文件即将移至 `docs/exec-plans/completed/`。
...
#### P1-1. 批量 INSERT(`unnest`) — ✅ 已完成

**修改原因:** 之前 `src/lib/workspace-store.ts::writeNormalizedWorkspace` 用 9 个 for-loop 逐行 `INSERT`,每个 RTT 插 1 行;工作区规模线性放大写延迟

**实现方式:** `writeNormalizedWorkspace` 中所有 9 个子表写入已全部改用 `insertRowsWithUnnest`(→ `prepareUnnestArgs`)批量 INSERT 路径:
- `buildWorkspaceWriteRows` 将 workspace 展平为 9 组行数组
- `insertRowsWithUnnest` + `prepareUnnestArgs` 将每组合并为单条 `SELECT * FROM unnest(...)` 语句
- 覆盖所有子表:players / positions / scenarios / defenseAssignments / lineupSlots / games / innings / statLines / milestones
- 已有 `prepareUnnestArgs` + 5 个单元测试,且 `insertRowsWithUnnest` 在生产代码中有 9 个内部调用方

**验证:**
- ✅ `npm test` 全绿(325 pass)
- ✅ 写入后 `getOrCreateWorkspaceSnapshot` 返回结果与字段级一致
- ✅ 小工作区(12 球员)写延迟 20-25ms(实测)
